 ...
 /**#@-*/
   ...
   */
    function login($email, $password, $show_confirmation = 1) {
        ...
        if     ($tmp_login_email == "") $func->information(t('Bitte gib deine E-Mail-Adresse oder deine Lansuite-ID ein.'), '', 1);
		...
		if (array_key_exists('pwmethod',$user) && ($user["pwmethod"] != "scram-sha1" or $user['password']=== md5($password))) {
		    ...
		    $scram = generate_scram($password,4096,'sha1');
                ...
                if ($show_confirmation) { 
                  // Print Loginmessages
                  if ($_GET['mod']=='auth' AND $_GET['action'] == 'login') $auth_backlink = "index.php?mod=home";
                      else $auth_backlink = "";
                  ...
                  $db->free_result($res);
                  if ($msg != '') $func->information('<b>'. t('Fehlerhafte Logins') .'</b>'. HTML_NEWLINE .t('Es wurden fehlerhafte Logins seit deinem letzten erfolgreichen Login durchgeführt.'). HTML_NEWLINE . HTML_NEWLINE . $msg, NO_LINK, 1);
